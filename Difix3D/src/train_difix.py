...
#     if mse == 0:
#         return torch.tensor(100.0)  # perfect
...
def compute_psnr(pred, target, max_val=1.0):
    ...
    if pred.numel() == 0 or target.numel() == 0:
        print("Empty tensor detected in PSNR computation.")
        return torch.tensor(float('nan'))

    mse = F.mse_loss(pred, target, reduction='mean')

    if torch.isnan(mse) or torch.isinf(mse):
        print(f"Invalid MSE value: {mse.item()}")
        return torch.tensor(float('nan'))
...
def main(args):
    ...
    # layers_to_opt = list(set(layers_to_opt))
    ####################################################
    ...
    for p in net_difix.unet.parameters():
        if p.requires_grad:
    ...
    # skip convs
    for conv in [
        net_difix.vae.decoder.skip_conv_1,
        net_difix.vae.decoder.skip_conv_2,
        net_difix.vae.decoder.skip_conv_3,
        net_difix.vae.decoder.skip_conv_4,
    ]:
    ...
    layers_to_opt = list(OrderedDict.fromkeys(layers_to_opt))
    print("Trainable params:", sum(p.numel() for p in layers_to_opt))
    ...
    try:
        dataset_val = PairedDataset(dataset_path=args.dataset_path, split="test", tokenizer=net_difix.tokenizer)
        if hasattr(dataset_val, "img_names"):
            random.Random(42).shuffle(dataset_val.img_names)
        else:
            print("[Warning] dataset_val does not contain 'img_names'; skipping shuffle.")
    ...
    dl_val = (
        torch.utils.data.DataLoader(dataset_val, batch_size=1, shuffle=False, num_workers=0)
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                with torch.no_grad():
                    # [-1, 1] → [0, 1] 정규화
                    x_tgt_norm = (x_tgt.clamp(-1, 1) + 1) / 2
                    x_tgt_pred_norm = (x_tgt_pred.clamp(-1, 1) + 1) / 2
                    psnr = compute_psnr(x_tgt_pred_norm, x_tgt_norm)
                    ssim = piq.ssim(x_tgt_pred_norm, x_tgt_norm, data_range=1.0).item()

                ssim = piq.ssim(
                    (x_tgt_pred.clamp(-1, 1) + 1) / 2,
                    (x_tgt.clamp(-1, 1) + 1) / 2,
                    data_range=1.0,
                ).item()
                ...
                if args.lambda_gram > 0:
                    if global_step > args.gram_loss_warmup_steps:
                        ...
                        crop_h = min(H, max(64, int(H * 0.8)))
                        crop_w = min(W, max(64, int(W * 0.8)))

                        print(f"[DEBUG] Crop size: {crop_h}x{crop_w}, Image size: {H}x{W}")
                        if H < crop_h or W < crop_w:
                            print(f"❌ Image is smaller than crop. Skipping crop. H={H}, W={W}, crop_h={crop_h}, crop_w={crop_w}")
                            continue  # 또는 loss_gram 없이 진행
                ...
                if not torch.isfinite(loss):
                    print(f"❌ Loss is NaN or Inf: {loss.item()} — skipping step")
                ...
                if not any_grad:
                    print("⚠️ No gradients found — skipping step.")
                    optimizer.zero_grad(set_to_none=True)
                    scaler.update()
                    continue
                ...
                if accelerator.sync_gradients:
                    try:
                        ...
                        torch.nn.utils.clip_grad_norm_(layers_to_opt, args.max_grad_norm)
                    except Exception as e:
                        print("⚠️ Gradient unscale error — skipping step:", e)
                        optimizer.zero_grad(set_to_none=True)
                        # DON'T call scaler.update() here
                        continue  
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_training_epochs", type=int, default=10) # default = 10
