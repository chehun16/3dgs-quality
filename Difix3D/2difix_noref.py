...
# ====== 사용자 설정 ======
TRANSFORMS_JSON = "transforms_train.json"   # 읽을 transforms 파일
...
def main():
    ...
    # file_path 목록(참조용)
    file_list = [to_abs(fr["file_path"]) for fr in frames]

    train = {}
    for i, fr in enumerate(frames):
        img = to_abs(fr["file_path"])
        # target 이미지: 기본은 동일 경로, TARGET_DIR 지정 시 해당 폴더로 치환
        tgt = remap_to_target(img)

        # ref 이미지: ref_idx 있으면 그 인덱스의 이미지, 없으면 자기 자신
        ref_idx = fr.get("ref_idx", None)
        if isinstance(ref_idx, int) and 0 <= ref_idx < len(file_list):
            ref = file_list[ref_idx]
        else:
            ref = img
